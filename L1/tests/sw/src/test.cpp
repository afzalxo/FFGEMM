...
#elif BLAS_L3
	if (l_curInstr.m_opClass == B3_OP_CLASS) {
	    uint32_t l_m, l_n, l_k;
	    int err = 0;
	    BLAS_dataType *l_a = nullptr, *l_b = nullptr, *l_c = nullptr, *l_res = nullptr, *l_resRef = nullptr;
	    l_gen.decodeB3Instr(l_curInstr, l_m, l_n, l_k, l_a, l_b, l_resRef);
	    l_c = new BLAS_dataType[l_m * l_n];
	    l_res = new BLAS_dataType[l_m * l_n];
	    // l_gen.printProgram();
	    for (int p = 0; p < l_m; p++) {
		for (int l = 0; l < l_n; l++) {
		  l_res[p * l_n + l] = 0;
	    ...
	    uut_top(l_m, l_n, l_k, l_a, l_b, l_c, l_res);
	    l_return = l_return && compare(l_m * l_n, l_res, l_resRef, err, true);
	    delete[] l_res;
	    if (!l_return) break;
	}
#else
