...
int main(int argc, char** argv) {
    ...
    if (l_write) {
        ...
        vector<vector<BLAS_dataType> > l_a, l_b, l_res, l_x, l_y, l_aRes, l_xRes, l_yRes;
        l_a.resize(BLAS_maxNumInstrs);
	l_b.resize(BLAS_maxNumInstrs);
	l_res.resize(BLAS_maxNumInstrs);
        ...
        while (l_argIdx < argc) {
            ...
            if (l_opCode <= B1_MaxOpCode) {
                ...
                assert(l_status == XFBLAS_STATUS_SUCCESS);
            } else if (l_opCode <= B3_MaxOpCode) {
		uint32_t l_m = stoi(argv[l_argIdx++]);
		uint32_t l_n = stoi(argv[l_argIdx++]);
		uint32_t l_k = stoi(argv[l_argIdx++]);
		string l_handleA(argv[l_argIdx++]);
		string l_handleB(argv[l_argIdx++]);
		string l_handleRes(argv[l_argIdx++]);
		to_upper(l_handleA);
		to_upper(l_handleB);
		to_upper(l_handleRes);
		uint8_t* l_aPtr = nullptr;
		uint8_t* l_bPtr = nullptr;
		uint8_t* l_resPtr = nullptr;
		initMat(l_handleA, l_opCode, l_m, l_k, 0, 0, l_a[l_idx], l_aPtr);
		initMat(l_handleB, l_opCode, l_k, l_n, 0, 0, l_b[l_idx], l_bPtr);
		initMat(l_handleRes, l_opCode, l_m, l_n, 0, 0, l_res[l_idx], l_resPtr);
		xfblasStatus_t l_status = l_gen.addB3Instr(l_opName, l_m, l_n, l_k, l_aPtr, l_bPtr, l_resPtr);
		assert(l_status == XFBLAS_STATUS_SUCCESS);
	    } else {
        ...
        uint32_t l_m, l_n, l_k, l_kl, l_ku;
        BLAS_dataType l_alpha, l_beta;
        BLAS_resDataType l_resGolden;
        BLAS_dataType *l_a, *l_b, *l_res, *l_x, *l_y, *l_aRes, *l_xRes, *l_yRes;
        BLAS_dataType l_aVal = 0;
	BLAS_dataType l_bVal = 0;
	BLAS_dataType l_resVal = 0;
	...
	l_res = &l_resVal;
        ...
        for (unsigned int i = 0; i < l_instrs.size(); ++i) {
            ...
            } else if (l_curInstr.m_opClass == B3_OP_CLASS) {
		l_gen.decodeB3Instr(l_curInstr, l_m, l_n, l_k, l_a, l_b, l_res);
		cout << "m=" << l_m << "  n=" << l_n << "  k=" << l_k << endl;
		outputMat("A:", l_curInstr.m_opCode, l_m, l_k, 0, 0, l_a);
		outputMat("B:", l_curInstr.m_opCode, l_k, l_n, 0, 0, l_b);
		outputMat("Res:", l_curInstr.m_opCode, l_m, l_n, 0, 0, l_res);
	    } else {
		assert(0); // Unknown opcode
	    }
	}
