...
class DataGenerator:
    ...
    def zeros_matrix(self, size: tuple):
        if not len(size) == 2:
            raise OP_ERROR("Matrix size error.")
        return np.zeros(size, dtype=self.dataType)

    def inc_matrix(self, size: tuple):
        if not len(size) == 2:
            raise OP_ERROR("Matrix size error.")
        m, n = size
        a = np.zeros(size, dtype=self.dataType)
        for i in range(m):
            for j in range(n):
                a[i][j] = (i * n + j) % self.maxValue
        return a

    def inc_symmetric_matrix(self, size: tuple):
        m, n = size
        a = np.zeros(size, dtype=self.dataType)
        for i in range(m):
            for j in range(n):
                a[i][j] = (i * n + j) % self.maxValue
        return np.array((a + a.transpose()) / 2, dtype=self.dataType)
...
class BLAS_L2(OP):
    ...
    def parse(opName, maxV, minV):
        try:
            ...
            raise OP_ERROR("BLAS_L2: %s is not defined." % opName)
...
class BLAS_L3(OP):
    ...
    def parse(opName, maxV, minV):
        try:
            op = eval(opName)(BLAS_L3(opName, maxV, minV))
            return op
        except BaseException:
            raise OP_ERROR("BLAS_L3: %s is not defined." % opName)
    ...
    def __init__(self, name, maxV, minV):
        ...
        self.n = 0
        self.k = 0
        # self.interfaceList = ('p_m', 'p_n', 'p_k', 'p_A', 'p_B', 'p_C', 'p_R')
        self.interfaceList = {'p_m': int, 'p_n': int, 'p_k': int, 'p_A': list, 'p_B': list, 'p_C': list, 'p_R': list}
    ...
    def features(self):
        ...
        features['Mat. Size m'] = self.m
        features['Mat. Size n'] = self.n
        features['Mat. Size k'] = self.k
        features['No.OPs'] = self.m * self.n * self.k
        return features

    def time(self, parallel, clock):
        return self.m * self.n * self.k * clock / parallel
    ...
    def addInstr(self, blas_gen, register):
        for a, b, c, res in register:
            blas_gen.addB3Instr(self.name, self.m, self.n, self.k,
                                a, b, res)

    def setSize(self, mnk):
        self.matrixDim = tuple(mnk)
        self.m = mnk[0]
        self.n = mnk[1]
        self.k = mnk[2]
        self.sizeStr = "m%d-n%d-k%d" % (self.m, self.n, self.k)
        self.memorySize = self.m * self.k + self.k * self.n + self.m * self.n

    def paramTCL(self, f):
        f.write('   L3 true\n ')
        f.write('   opName "%s"\n ' % self.name)
        f.write('   matrixSizeA %d\n ' % (self.m * self.k))
        f.write('   matrixSizeB %d\n ' % (self.k * self.n))
        f.write('   matrixSizeC %d\n ' % (self.m * self.n))
        f.write('   matrixSizeR %d\n ' % (self.m * self.n))
        f.write('   m %d\n ' % self.m)
        f.write('   n %d\n ' % self.n)
        f.write('   k %d\n ' % self.k)
        #f.write('   memorySize %d\n ' % self.memorySize)
    ...
    def test(self, runTest):
        matDimList = runTest.profile['matrixDims']
        dataTypeList = runTest.dataTypes
        self.PE = runTest.parEntries
        for dataType in dataTypeList:
            self.setDtype(dataType)
            runTest.build()
            self.setSize(tuple(matDimList))
            self.specTest(runTest)
...
class gemm(BLAS_L3):
    def __init__(self, blas_l3: BLAS_L3):
        self.copyConstructor(blas_l3)

    def compute(self):
        BLAS_L3.compute(self)
        matrixA = self.dataGen.matrix((self.m, self.k))
        matrixB = self.dataGen.matrix((self.k, self.n))
        matrixC = self.dataGen.zeros_matrix((self.m, self.n))
        res = np.matmul(matrixA, matrixB, dtype=self.dataGen.dataType)
        # print(np.transpose(matrixA).shape)
        matrixA = np.transpose(matrixA).copy()
        return matrixA, matrixB, matrixC, res
