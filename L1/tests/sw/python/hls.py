...
class Parameters:
    ...
    def setPath(self, path):
        self.path = path  # Path to profile.json


class HLS:
    def __init__(self, tclPath, b_csim, b_syn, b_cosim, b_benchmark):

        self.tcl = tclPath  # Path to build/run_hls.tcl
        self.csim = b_csim
        self.syn = b_syn
        self.cosim = b_cosim
    ...
    def execution(self, binFile, logFile, workDir='.', b_print=False):
        ...
        if not b_print:
            print(
                "\nOP %s: vitis_hls stdout print is hidden." %
                self.params.op.name)
        print('\nExecuting: %s' % commandLine)
    ...
    def generateParam(self, fileparams):
        ...
        print('Generating parameters file %s' % self.paramFile)
    ...
    def generateDirective(self, directivePath):
        ...
        with open(self.directive, 'w') as f:
            for inface, dtype in self.params.op.interfaceList.items():
