...
class BLAS_GEN:
    ...
    def addB3Instr(self, p_opName, p_m, p_n, p_k, p_a, p_b, p_Res):
        func = self.lib.addB3Instr
        func.argtypes = [
            ct.c_void_p,
            ct.c_char_p,
            ct.c_int,
        ...
        status = func(
            ...
            p_k,
            self._getPointer(p_a),
            self._getPointer(p_b),
            self._getPointer(p_Res))
        if status > 0:
            raise BLAS_ERROR(
                self.status[status],
                "Add BLAS_L3 instruction failed.")
