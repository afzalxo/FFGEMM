...
class ParamB3 {
   ...
   public:
    ParamB3() { m_aStore = GEM; }

   public:
    void print(ostream& os) {
        os << "m=" << m_m << " n=" << m_n << " k=" << m_k << "\n"; 
        showMatrix<t_DataType>(os, m_m, m_k, m_aAddr, "A:");
	showMatrix<t_DataType>(os, m_k, m_n, m_bAddr, "B:");
	showMatrix<t_DataType>(os, m_m, m_n, m_resAddr, "Res (AxB):");
    }
...
template <typename T1, unsigned int T2>
ostream& operator<<(ostream& os, ParamB3<T1, T2>& p_val) {
    p_val.print(os);
    return (os);
}
