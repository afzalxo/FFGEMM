...
class GenBin {
   public:
    ...
    typedef typename Program<t_HPPandleType,
			     ...
			     t_InstrPageIdx,
			     t_ParamPageIdx,
			     t_StatsPageIdx>::ParamB3Type ParamB3Type;

    typedef typename ParamB3<t_DataType, t_ParEntries>::MatStoreType MatStoreTypeB3;
    typedef typename ParamB2<t_DataType, t_ParEntries>::MatStoreType MatStoreTypeB2;
   ...
   public:
    ...
    static const size_t ParamB3Bytes = Program<t_HPPandleType,
		 			       t_DataType,
					       t_ResDataType,
					       t_MemWidthBytes,
					       t_ParEntries,
   ...
   public:
    ...
    xfblasStatus_t addB2Instr(string p_opName,
        ...
        if ((l_status == XFBLAS_STATUS_SUCCESS) && (l_opCode32 > B1_MaxOpCode) &&
            ...
            MatStoreTypeB2 l_store;
            uint32_t l_rows = 0;
            uint32_t l_entries = 0;
            switch (l_opCode) {
                case SBMV:
                    if (p_kl == 0) {
                        l_store = MatStoreTypeB2::SBM_UP;
                        l_rows = p_ku + 1;
                    } else {
                        l_store = MatStoreTypeB2::SBM_LO;
                        l_rows = p_kl + 1;
                    }
                    break;
                case SPMV:
                    assert(p_m == p_n);
                    if (p_kl == 0) {
                        l_store = MatStoreTypeB2::PM_UP;
                        assert(p_ku == p_n);
                    } else {
                        l_store = MatStoreTypeB2::PM_LO;
                        assert(p_kl == p_n);
                    }
                ...
                case TBMV:
                    if (p_kl == 0) {
                        l_store = MatStoreTypeB2::TBM_UP;
                        l_rows = p_ku + 1;
                    } else {
                        l_store = MatStoreTypeB2::TBM_LO;
                        l_rows = p_kl + 1;
                    }
                    break;
                case TPMV:
                    ...
                    break;
                case GBMV:
                    l_store = MatStoreTypeB2::GBM;
                    l_rows = p_ku + p_kl + 1;
                    break;
                default:
                    l_store = MatStoreTypeB2::GEM;
                    break;
            }
            l_param.m_aStore = l_store;

            if ((l_store == MatStoreTypeB2::PM_UP) || (l_store == MatStoreTypeB2::PM_LO)) {
    ...
    xfblasStatus_t addB3Instr(string p_opName,
                              uint32_t p_m,
                              uint32_t p_n,
                              uint32_t p_k,
                              void* p_a,
        ...
        xfblasStatus_t l_status = m_opFinder.getOpCode(p_opName, l_opCode32);
        uint16_t l_opClass, l_opCode;
        if ((l_status == XFBLAS_STATUS_SUCCESS) && (l_opCode32 > B2_MaxOpCode) &&
            (l_opCode32 <= B3_MaxOpCode)) { // BLAS L3 operations
            l_opClass = B3_OP_CLASS;
            l_opCode = l_opCode32;
            ...
            MatStoreTypeB3 l_store = MatStoreTypeB3::GEM;
            l_param.m_aStore = l_store;

	    l_status = regMem(p_m * p_k * sizeof(t_DataType), p_a, l_param.m_aAddr);
	    l_status = regMem(p_k * p_n * sizeof(t_DataType), p_b, l_param.m_bAddr);
	    l_status = regMem(p_m * p_n * sizeof(t_DataType), p_res, l_param.m_resAddr);

            uint8_t* l_instrVal = reinterpret_cast<uint8_t*>(&l_instr);
            uint8_t* l_paramVal = reinterpret_cast<uint8_t*>(&l_param);

            m_program.addInstr(l_instrVal, l_paramVal, ParamB3Bytes);
            return (l_status);
        } else {
            return (XFBLAS_STATUS_INVALID_OP);
        }
    ...
    void decodeB3Instr(const Instr& p_instr,
		       ...
		       t_DataType*& p_b,
		       t_DataType*& p_res) {
	m_program.decodeB3Instr(p_instr, p_m, p_n, p_k, p_a, p_b, p_res);
    }
