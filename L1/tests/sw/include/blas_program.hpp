...
class Program {
   public:
    ...
    typedef ParamB3<t_DataType, t_ParEntries> ParamB3Type;
    typedef typename ParamB3<t_DataType, t_ParEntries>::MatStoreType MatStoreType;
   ...
   public:
    ...
    static const size_t ParamB3Bytes = sizeof(ParamB3Type);
   ...
   public:
    ...
    ParamB3Type getB3Param() {
	ParamB3Type l_param;
	uint8_t* l_baseInstrAddr = getBaseInstrAddr();
	memcpy((uint8_t*)&l_param, l_baseInstrAddr + m_currParamOff, ParamB3Bytes);
	m_currParamOff += ParamB3Bytes;
	while (m_currParamOff % t_MemWidthBytes != 0) {
	    m_currParamOff++;
	}
	return (l_param);
    ...
    void writeB3Param(ofstream& p_of,
                      ParamB3Type& p_param,
                      ifstream::pos_type& p_ofParamPos,
                      ifstream::pos_type& p_ofDataPos) {
        uint32_t l_m = p_param.m_m;
        uint32_t l_n = p_param.m_n;
	uint32_t l_k = p_param.m_k;
        size_t l_matADataBytes = l_m * l_k * sizeof(t_DataType);
        size_t l_matBDataBytes = l_k * l_n * sizeof(t_DataType);
        size_t l_matRDataBytes = l_m * l_n * sizeof(t_DataType);
        size_t l_matAPaddingBytes =
            ((l_matADataBytes % t_PageSizeBytes) != 0) ? (t_PageSizeBytes - (l_matADataBytes % t_PageSizeBytes)) : 0;
        size_t l_matBPaddingBytes =
            ((l_matBDataBytes % t_PageSizeBytes) != 0) ? (t_PageSizeBytes - (l_matBDataBytes % t_PageSizeBytes)) : 0;
        size_t l_matRPaddingBytes =
            ((l_matRDataBytes % t_PageSizeBytes) != 0) ? (t_PageSizeBytes - (l_matRDataBytes % t_PageSizeBytes)) : 0;

        ifstream::pos_type l_ofPos = p_ofDataPos;
        writeData(l_matADataBytes, l_matAPaddingBytes, p_param.m_aAddr, l_ofPos, p_of);
        writeData(l_matBDataBytes, l_matBPaddingBytes, p_param.m_bAddr, l_ofPos, p_of);
        writeData(l_matRDataBytes, l_matRPaddingBytes, p_param.m_resAddr, l_ofPos, p_of);

        p_ofDataPos = l_ofPos;

        p_of.seekp(p_ofParamPos);
        p_of.write((char*)&p_param, ParamB3Bytes);
        size_t l_paddingBytes =
            ((ParamB3Bytes % t_MemWidthBytes) != 0) ? (t_MemWidthBytes - (ParamB3Bytes % t_MemWidthBytes)) : 0;
        writePaddingBytes(l_paddingBytes, p_of);
        p_ofParamPos += (ParamB3Bytes + l_paddingBytes);
    }
    ...
    xfblasStatus_t write2BinFile(const string& p_fileName) {
        ...
        if (l_of.is_open()) {
            ...
            for (unsigned int i = 0; i < m_numInstrs; ++i) {
                ...
                } else if (l_instrs[i].m_opClass == B3_OP_CLASS) {
		    ParamB3Type l_param = getB3Param();
		    writeB3Param(l_of, l_param, l_ofParamPos, l_ofDataPos);
		}
            }
            l_of.close();
        } else {
    ...
    xfblasStatus_t readFromBinFile(string p_fileName) {
		...
		l_param.m_aAddr =
			(l_param.m_aAddr != 0) ? reinterpret_cast<uint64_t>(l_baseInstrAddr + l_param.m_aAddr) : 0;
		l_param.m_bAddr =
			(l_param.m_bAddr != 0) ? reinterpret_cast<uint64_t>(l_baseInstrAddr + l_param.m_bAddr) : 0;
		l_param.m_resAddr =
			(l_param.m_resAddr != 0) ? reinterpret_cast<uint64_t>(l_baseInstrAddr + l_param.m_resAddr) : 0;
		memcpy(l_baseInstrAddr + m_currParamOff, (uint8_t*)&l_param, ParamB3Bytes);
		m_currParamOff += ParamB3Bytes;
	    }
            while (m_currParamOff % t_MemWidthBytes != 0) {
                m_currParamOff++;
    ...
    void decodeB3Instr(const Instr& p_instr,
		       uint32_t& p_m,
		       uint32_t& p_n,
		       uint32_t& p_k,
		       t_DataType*& p_a,
	...
	uint8_t* l_paramAddr = l_baseAddr + p_instr.m_paramOff;
	ParamB3Type l_param;
	memcpy((uint8_t*)&l_param, l_paramAddr, ParamB3Bytes);
	p_m = l_param.m_m;
	p_n = l_param.m_n;
	p_k = l_param.m_k;
	p_a = reinterpret_cast<t_DataType*>(l_param.m_aAddr);
	p_b = reinterpret_cast<t_DataType*>(l_param.m_bAddr);
	p_res = reinterpret_cast<t_DataType*>(l_param.m_resAddr);
    }
