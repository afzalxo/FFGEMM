...
template <typename t_FloatType,    // matrix A, B entry data type
          ...
          unsigned int t_aColMemWords = 1, // number of memory words in one row of the matrix A buffer / B row buffer
   ...
   public:
     ...
     */
     void GemmReadAB(   MemIntType* l_aAddr,
			...
			MemStream& s_lhs,
		        MemStream& s_rhs) {
	 MemWideType buffer_a[2][2][t_aMH][t_aColMemWords];
...
#pragma HLS ARRAY_PARTITION variable = buffer_a complete dim = 2
	 MemWideType buffer_b[2][2][t_bKD][t_bColMemWords];
...
gemmreadab_inner:
		   ...
		   t_subMatOps.ReadAndBufferComplete(l_bAddr, buffer_b);
		   t_subMatOps.ReadAndBufferComplete(l_aAddr, buffer_a);

		   t_subMatOps.ReadAndBuffer(l_bAddr, (l_aColBlock << 1) + 0, (l_bColBlock << 1) + 0, l_bWordLd, buffer_b[0][0]);
		   t_subMatOps.ReadAndBuffer(l_bAddr, (l_aColBlock << 1) + 1, (l_bColBlock << 1) + 1, l_bWordLd, buffer_b[1][1]);
		   t_subMatOps.ReadAndBuffer(l_aAddr, (l_aRowBlock << 1) + 0, (l_aColBlock << 1) + 0, l_aWordLd, buffer_a[0][0]);
		   t_subMatOps.ReadAndBuffer(l_aAddr, (l_aRowBlock << 1) + 1, (l_aColBlock << 1) + 1, l_aWordLd, buffer_a[1][1]);
		   t_subMatOps.AddFactors(buffer_b[0][0], buffer_b[1][1], s_rhs);
		   t_subMatOps.AddFactors(buffer_a[0][0], buffer_a[1][1], s_lhs);

		   t_subMatOps.ReadAndBuffer(l_aAddr, (l_aRowBlock << 1) + 1, (l_aColBlock << 1) + 0, l_aWordLd, buffer_a[1][0]);
		   t_subMatOps.BlockBufferToStream(buffer_b[0][0], s_rhs);
		   t_subMatOps.AddFactors(buffer_a[1][0], buffer_a[1][1], s_lhs);

		   t_subMatOps.ReadAndBuffer(l_bAddr, (l_aColBlock << 1) + 0, (l_bColBlock << 1) + 1, l_bWordLd, buffer_b[0][1]);
		   t_subMatOps.AddFactors(buffer_b[0][1], buffer_b[1][1], s_rhs);
		   t_subMatOps.BlockBufferToStream(buffer_a[0][0], s_lhs);

		   t_subMatOps.ReadAndBuffer(l_bAddr, (l_aColBlock << 1) + 1, (l_bColBlock << 1) + 0, l_bWordLd, buffer_b[1][0]);
		   t_subMatOps.ReadAndBuffer(l_aAddr, (l_aRowBlock << 1) + 0, (l_aColBlock << 1) + 1, l_aWordLd, buffer_a[0][1]);
		   t_subMatOps.AddFactors(buffer_b[1][0], buffer_b[0][0], s_rhs);
		   t_subMatOps.BlockBufferToStream(buffer_a[1][1], s_lhs);

		   t_subMatOps.BlockBufferToStream(buffer_b[1][1], s_rhs);
		   t_subMatOps.AddFactors(buffer_a[0][0], buffer_a[0][1], s_lhs);

		   t_subMatOps.AddFactors(buffer_b[0][0], buffer_b[0][1], s_rhs);
		   t_subMatOps.AddFactors(buffer_a[1][0], buffer_a[0][0], s_lhs);

		   t_subMatOps.AddFactors(buffer_b[1][0], buffer_b[1][1], s_rhs);
		   t_subMatOps.AddFactors(buffer_a[0][1], buffer_a[1][1], s_lhs);
	       }
	    }
	 }
    }

    void ClearBuffer(MemWideType p_buffer[2][2][t_aMH][t_aColMemWords]) {
#pragma HLS INLINE
...
#pragma HLS DEPENDENCE variable = p_buffer1 array inter RAW false
	...
	MemWideType l_buffer[2][2][t_aMH][t_aColMemWords];
...
#pragma HLS ARRAY_PARTITION variable = l_buffer complete dim = 2
...
#pragma HLS PIPELINE II = t_aColMemWords
	        	for (int j = 0; j < t_aColMemWords; ++j) {
		    	    p_ostream.write(l_buffer[i][j][k][l]);
	        	}
		    }
		}
	    }
...
#pragma HLS bind_storage variable = l_rhs type = fifo impl = uram
	...
	// GemmReadAB_Auto(p_aAddr, p_bAddr, p_aColBlocks, p_aRowBlocks, p_bColBlocks, p_aLd, p_bLd, l_lhs, l_rhs);
	ReadThenStream(p_aAddr, p_bAddr, p_aColBlocks, p_aRowBlocks, p_bColBlocks, p_aLd, p_bLd, l_lhs, l_rhs);
