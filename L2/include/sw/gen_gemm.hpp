...
template <typename T>
void add_ref(
    DenseMat<T>& p_A, DenseMat<T>& p_B, DenseMat<T>& p_C) {
    assert(p_A.rows() == p_C.rows());
    assert(p_A.cols() == p_B.rows());
    assert(p_B.cols() == p_C.cols());
    for (unsigned int row = 0; row < p_C.rows(); ++row) {
        for (unsigned int col = 0; col < p_C.cols(); ++col) {
	    T l_val = p_A.getVal(row, col) + p_B.getVal(row, col);
            T l_entry = (T)(l_val);
            p_C.getVal(row, col) = l_entry;
        }
    }
...
template <typename T>
void transpose_ref(DenseMat<T>&p_A) {
...
class GenGemm {
   public:
    ...
    bool check(unsigned int p_M,
               ...
               unsigned int p_LdX) {
        bool ok = true;
        const unsigned int l_Edge = BLAS_memWidth;
        const unsigned int l_mMin = BLAS_gemmMBlocks * BLAS_memWidth;
        const unsigned int l_kMin =
            BLAS_gemmKBlocks * BLAS_memWidth; // BLAS_gemmKBlocks must >= 2, due to kernel Cout control to save area
        const unsigned int l_nMin = BLAS_gemmNBlocks * BLAS_memWidth;
    ...
    void addInstr(ProgramType& p_Program,
	    ...
	    // add_ref<BLAS_dataType>(l_matA, l_matB, l_matC);
	    // transpose_ref<BLAS_dataType>(l_matA);
        }
