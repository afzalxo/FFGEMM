...
class DenseMat {
   ...
   public:
	...
	for (unsigned int row = 0; row < rows(); ++row) {
		...
		std::swap(getVal(row, col), getVal(col, row));
	    }
	}
    ...
    bool cmp(float p_TolRel, float p_TolAbs, DenseMat& p_Ref) {
        ...
        for (unsigned int row = 0; row < rows(); ++row) {
            for (unsigned int col = 0; col < cols(); ++col) {
                ...
                bool l_ok = xf::blas::cmpVal<T>(p_TolRel, p_TolAbs, vRef, v, l_Prefix, l_exactMatch, 0);  // Final argument is verbosity
                ok = ok && l_ok;
                if (l_exactMatch) {
                    l_numExactMatches++;
                }
...
typedef DenseMat<BLAS_dataType> MatType;
