...
class MemMatrixShape {
   ...
   public:
    MemMatrixShape() {}
    MemMatrixShape(unsigned int p_Offset,
                   unsigned int p_Rows,
                   unsigned int p_Cols,
                   unsigned int p_Ld,
...
inline std::ostream& operator<<(std::ostream& os, MemMatrixShape& p_Val) {
    p_Val.print(os);
    return (os);
}
...
class TranspArgs {
   public:
    MemMatrixShape m_Src, m_Dst;

   public:
    TranspArgs() {}
    TranspArgs(MemMatrixShape p_Src, MemMatrixShape p_Dst) : m_Src(p_Src), m_Dst(p_Dst) {}
};

////////////////////////////  KARGS module  ////////////////////////////

template <typename t_DataType, // Interface type for client (instead of ap_uint)
          unsigned int t_MemWidthFloats,
          unsigned int t_InstrWidth, // For narrow DDR, >1 allows to fit the instruction (in MemWords)
          unsigned int t_ArgPipeline,
          unsigned int t_DataBitWidth = 8 * sizeof(t_DataType)>
class Kargs {
   public:
    typedef typename WideType<t_DataType, t_MemWidthFloats>::t_TypeInt MemFloatType;
    static const int t_MemWidthBits = t_DataBitWidth * t_MemWidthFloats * t_InstrWidth;
    class MemInstrType {
       private:
        MemFloatType v[t_InstrWidth];
...
#pragma HLS ARRAY_PARTITION variable = v complete dim = 1
        ...
        MemFloatType& operator[](int i) { return (v[i]); }
        void loadFromMem(MemFloatType* p_Addr, unsigned int p_Offset) {
            for (unsigned int i = 0; i < t_InstrWidth; ++i) {
                v[i] = p_Addr[p_Offset + i];
            }
        }
        void storeToMem(MemFloatType* p_Addr, unsigned int p_Offset) {
            for (unsigned int i = 0; i < t_InstrWidth; ++i) {
                p_Addr[p_Offset + i] = v[i];
            }
        }
    };
    typedef ap_uint<t_MemWidthBits> MemBitType;
    typedef enum {
        OpControl = 0x00,
        OpGemv = 0x01,
        OpGemm = 0x02,
   ...
   private:
    ...
    void loadVal(T& var) {
        ...
        if (l_endBit >= t_MemWidthBits) {
            m_WordPos++;
            m_BitPos = 0;
            l_endBit = m_BitPos + l_sizeBits - 1;
        }
        assert(m_WordPos <= 0);
        assert(l_endBit <= t_MemWidthBits);
    ...
    MemBitType wideFloatToBits(MemInstrType p_Val) {
        MemBitType l_Val;
        for (unsigned int i = 0; i < t_InstrWidth; ++i) {
#pragma HLS unroll
            unsigned int l_instrBits = t_MemWidthFloats * t_DataBitWidth;
            l_Val.range((i + 1) * l_instrBits - 1, i * l_instrBits) = p_Val[i];
        }
        return (l_Val);
    }

    MemInstrType wideBitsToFloat(MemBitType p_Val) {
        MemInstrType l_Val;
        for (unsigned int i = 0; i < t_InstrWidth; ++i) {
#pragma HLS unroll
            unsigned int l_instrBits = t_MemWidthFloats * t_DataBitWidth;
...
#pragma HLS INLINE
    ...
    OpType loadFromInstr(MemInstrType p_Val) {
    ...
    void storeToInstr(MemInstrType& p_Val) { p_Val = wideBitsToFloat(m_Flat); }

    OpType load(MemFloatType* p_Addr, unsigned int p_Pc) {
        MemInstrType l_val;
        l_val.loadFromMem(p_Addr, p_Pc);
        return (loadFromInstr(l_val));
    }

    void store(MemFloatType* p_Addr, unsigned int p_Pc) {
        MemInstrType l_val = wideBitsToFloat(m_Flat);
        l_val.storeToMem(p_Addr, p_Pc);
    }
