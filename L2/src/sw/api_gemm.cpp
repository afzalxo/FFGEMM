...
int main(int argc, char** argv) {
    ...
    int l_argIdx = 1;
    bool enable_output_comparison = false;
    ...
    unsigned int l_memW = BLAS_memWidth;
    // the smallest matrices for flow testing
    unsigned int l_M = l_memW * BLAS_gemmMBlocks, l_K = l_memW * BLAS_gemmKBlocks, l_N = l_memW * BLAS_gemmNBlocks;
    if (argc > ++l_argIdx) {
    ...
    if (enable_output_comparison) {
        std::cout << "Constructing Golden Program... " << std::endl;
    ...
    l_timeMsAt100pctEff = l_Parallel_Ops / 2 / BLAS_memWidth / BLAS_memWidth / (l_boardFreqMHz * 1e6) * 1e3;
    ...
    std::cout << std::string("DATA_CSV:,MemWidth,Freq,M,K,N,") + "Ops,KernelCycles," + "TimeKernelMs,TimeApiMs," +
                     "EffKernelPct,EffApiPct," + "PerfKernelTops,PerfApiTops\n"
              << "DATA_CSV:," << BLAS_memWidth << "," << l_boardFreqMHz << "," << l_M << "," << l_K << "," << l_N << ","
