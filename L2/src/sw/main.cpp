...
std::vector<MemType, aligned_allocator<MemType> > loadBinFile(std::string p_BinFileName) {
    std::vector<MemType, aligned_allocator<MemType> > l_memVec;
    // Bin file existence
    ...
    if (l_if.is_open()) {
        ...
        size_t l_binFileSizeInMemWords = l_binFileSize / sizeof(MemType);
        assert(l_binFileSize % sizeof(MemType) == 0);

        // Bin file storage
        // l_memVec.reserve(l_binFileSizeInMemWords);
        l_memVec.resize(l_binFileSizeInMemWords);
        MemType* l_mem = &l_memVec[0];
...
bool writeBinFile(std::string p_BinFileName, std::vector<MemType, aligned_allocator<MemType> >& p_MemVec) {
    bool ok = false;
    std::ofstream l_of(p_BinFileName.c_str(), std::ios::binary);
    if (l_of.is_open()) {
        size_t l_sizeBytes = sizeof(MemType) * p_MemVec.size();
...
int main(int argc, char** argv) {
    ...
    // Load the bin file
    std::vector<MemType, aligned_allocator<MemType> > l_memVec[BLAS_numKernels];
...
#include <chrono>
    ...
    for (unsigned int i = 0; i < BLAS_numKernels; ++i) {
        l_memDesc[i].init(l_memVec[i].size() * sizeof(MemType) / BLAS_pageSizeBytes, l_memVec[i].data());
        assert(l_memVec[i].size() * sizeof(MemType) % BLAS_pageSizeBytes == 0);
    ...
    std::cout << "INFO: writing output data to " << l_binFileOut << "\n";
